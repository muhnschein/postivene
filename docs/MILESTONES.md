...
- [x] Packaging bugs found by auditing the spec against what Sailfish
      actually ships, and fixed (none of these could have worked as
      written):
  - `%{_arch}` expands to `arm` on an armv7hl build -- rpm canonicalises
    `armv7h*` to `arm` (`installplatform`: `armv7h*) CANONARCH=arm`), so
    `%install` looked for `vendor/deltachat-rpc-server/arm/` while
    `scripts/fetch-rpc-server.sh` writes `armv7hl/`. Now `%{_target_cpu}`.
  ...
  - Missing `BuildRequires: gcc-c++` (qmetaobject-rs compiles C++ glue via
    ...
    version floors.
  - `rust/Cargo.lock` was in the v4 lockfile format, which Cargo only
    learned to read in 1.78; Sailfish ships Rust **1.75.0**
    (`sailfishos/rust`, newest tag `1.75.0+git2`), whose cargo refuses it
    outright ("lock file version `4` was found, but this version of Cargo
    does not understand this lock file"). Converted to v3, and the
    workspace now declares `rust-version = "1.75"` so a future
    `cargo update` on a modern host cannot silently outrun the device
    toolchain. Verified: the whole workspace compiles under cargo 1.75.0.
...
- [ ] Verified with `sfdk build` inside the real Sailfish SDK (cross
      ...
      `scripts/build-rpm.sh` drives this in one command once an SDK and a
      build target exist. Note that `sfdk build` runs every SPEC section
      *except* `%prep` (it builds the working tree in place), so the
      gitignored `vendor/deltachat-rpc-server/<arch>/` binaries are picked
      up directly -- no tarball or vendoring step is needed for a device
