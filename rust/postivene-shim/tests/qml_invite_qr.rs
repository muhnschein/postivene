...
//! shown; the scan button pushes the scanner page, and what it hands back
//! is followed the way a pasted link is.
...
struct PageStackProbe {
    ...
    /// `push:ScanPage.qml|replace:ConversationPage.qml|...`
...
fn assert_page(steps: &[(&str, String)], calls: &[String]) {
    ...
    assert!(
        value("navigation").ends_with("replace:ConversationPage.qml|"),
        "following the scanned invite did not open the chat. {context}"
    );
}
